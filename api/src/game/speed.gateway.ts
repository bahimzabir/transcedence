...
export class SpeedGateway implements OnGatewayConnection, OnGatewayDisconnect {
	...
	async joinQueue(@ConnectedSocket() client: Socket) {
		...
		if (this.queue.length >= 2)
		{
			...
			await this.gameService.addSpeedRoom(room);
	...
	async handleDisconnect(client: Socket) {
		const room  = await this.gameService.findSpeedRoom(this.map.get(client));
		if (room) {
			room.done = true;
			const body = {
			...
			await this.gameService.removeSpeedRoom(room.roomName);
	...
	@Interval(8)
	async moveBall() {
		for (let room of this.gameService.getSpeedRooms()) {
