...
const Chat = () => {
    ...
    async function getRoomChannels() {
        try {
            const response = await axios.get("/api/users/me/chatrooms", {
                withCredentials: true,
            });
    ...
    async function getChannelmsg(id: any) {
        try {
            const res = await axios.get("/api/chat/getroomsmsg?id=" + id, {
                withCredentials: true,
            });
            return res.data;
        } catch (error) {
    ...
    async function receivemsg(dto: messagedto) {
        if (
            ...
            selectedChannelRef.current.id !== dto.roomid
        ) {
            const index = channels.findIndex(
                (channel) => channel.id === dto.roomid
            );
    ...
    async function getmemeberoom(roomID: number) {
        const res = await axios.get("/api/chat/roomMemebers?id=" + roomID, {
            withCredentials: true,
        });
        return res.data;
    }
    ...
    const leaveroom = async () => {
        try {
            await axios.post("/api/chat/leave", {
                roomid: selectedChannel?.id,
            });
    ...
    const deletechat = async () => {
        try {
            await axios.post("/api/chat/remove", {
                roomid: selectedChannel?.id,
            });
            notify("chat deleted");
            await Getmyrooms();
